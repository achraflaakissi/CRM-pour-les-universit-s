...
$conatctindirect = new contact_indirecte();
include($_SERVER['DOCUMENT_ROOT'].'/content/config.php');
...
$cis = $conatctindirect->Allcontactsindirect()->fetchAll();
...
if(isset($_POST['rech']))
{
    ...
    $sql='SELECT c.id, c.Civilite, CONCAT( u.nom, CONCAT(  " ", u.prenom ) ) AS Contact_Suivi_par, c.Etablissement, c.Nom, c.Prenom, `E-Mail` , c.GSM, c.Tel, c.Formation_Demmandee, c.Nature_de_Contact, c.Annee_Univ, c.Ville, c.Tel_Pere, c.Ville_Lycee, c.Tel_Mere, valide,c.Date,c.Marche
    ...
    if($_POST['villelycee']!="")
    {
        $sql.=' and Ville_Lycee like "%'.$_POST['villelycee'].'%" ';
    }
   ...
   if($_POST['datecontact']!="")
    {

        $sql.=' and c.Date  like "%'.$_POST['datecontact'].'%"';
    }
    if($_POST['marche']!="")
    {

        $sql.=' and c.Marche  like "%'.$_POST['marche'].'%"';
    }
    

    if($_POST['formationdemmande']=="" && $_POST['naturecontact']=="" &&  $_POST['etablissmeent']=="" && $_POST['villelycee']==""
        && $_POST['ville'] =="" &&  $_POST['gsm']=="" && $_POST['tel']=="" && $_POST['prenom']=="" && $_POST['nom']=="" && $_POST['datecontact']="" && $_POST['marche']="" )
    {
        $sql='SELECT c.id, c.Civilite, CONCAT( u.nom, CONCAT(  " ", u.prenom ) ) AS Contact_Suivi_par, c.Etablissement, c.Nom, c.Prenom, `E-Mail` , c.GSM, c.Tel, c.Formation_Demmandee, c.Nature_de_Contact, c.Annee_Univ, c.Ville, c.Tel_Pere, c.Ville_Lycee, c.Tel_Mere, valide,c.Date,c.Marche
            FROM contact_indirect c
            LEFT JOIN users u ON u.id = c.Contact_Suivi_par
...
if(isset($_POST['op']))
{
    ...
    if($_POST['op']=='s')
    {
            ...
            if($conatctindirect->supprimercontact($_POST['id']))
            {
                echo  $_POST['id'];
            }
    ...
    else if($_POST['op']=="up")
    {
            ...
            $conatctindirect->setId(md5($_POST['id']));
            $conatctindirect->setNom($_POST['nom']);
            $conatctindirect->setPrenom($_POST['prenom']);
            $conatctindirect->setTel($_POST['tel']);
            $conatctindirect->setGsm($_POST['gsm']);
            $conatctindirect->setEmail($_POST['email']);
            $conatctindirect->setAnneeUniv($_POST['anneeniv']);
            $conatctindirect->setVille($_POST['ville']);
            $conatctindirect->setTelPere($_POST['telpere']);
            $conatctindirect->setTelMere($_POST['telmere']);
            $conatctindirect->setNatureContact($_POST['naturecontact']);
            $conatctindirect->setFormationDemmandee($_POST['formationdemande']);
            $conatctindirect->setEtablissement($_POST['etablissement']);
            $conatctindirect->setContactsuivipar($_POST['contactsuivipar']);
            $conatctindirect->setDate($_POST['date_contact']);
            $conatctindirect->setValide($_POST['valide']);
            $conatctindirect->setMarche($_POST['marche']);
            echo $conatctindirect->Validercontactindirect();
            ...
            <div class="box-body" style="overflow: auto;">
                ...
                <form class="form-inline"  method="post">
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                <label >Ville Lycee</label>
                                <input type="text" class="form-control" name="villelycee" value="<?php if(isset($_POST['villelycee'])) echo $_POST['villelycee']; ?>" >
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="form-group">
...
<div class="col-md-2">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="datecontact" value="<?php if($_POST['datecontact']!="") echo $_POST['datecontact']; ?>">
                            </div>
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                <label >Marche</label>
                                <input type="text" class="form-control" value="<?php if($_POST['marche']!="") echo $_POST['marche']; ?>" name="marche" list="marche" />
                                <datalist id="marche" >
                ...
                <table id="contacts" class="table table-bordered table-striped">
                    ...
                    <thead>
                                  ...
                                  {
                                foreach($cis as $contactindir) { ?>
                                    <tr id="<?php echo $contactindir["id"]; ?>" >
                                        <td><input  id="<?php echo "check".$contactindir["id"]; ?>" type="checkbox" /></td>
                                        ...
                                        <td><span><?php echo $contactindir["Ville_Lycee"];?></span><input type="text" style="display: none;" value="<?php echo $contactindir["Ville_Lycee"]; ?>" /></td>
                                        ...
                                        <td class="withsel<?php echo $contactindir["id"]; ?>"><span><?php echo $contactindir["Formation_Demmandee"];?></span><input type="text" style="display: none;" value="<?php echo $contactindir["Formation_Demmandee"]; ?>" list="Formation_Demmandee"/><datalist id="Formation_Demmandee" >
                                         ...
                                         <td><span><?php echo date("d-m-Y", strtotime($contactindir["Date"]));?></span><input type="date" style="display:none;" value="<?php echo $contactindir["Date"];?>" /></td>
                                        <td><span><?php echo $contactindir["Marche"]; ?></span><input type="text" style="display: none;" value="<?php echo $contactindir["Marche"]; ?>" list="marche" /><datalist id="marche" >
                                                <?php echo $remplire->getDataList("getMarche"); ?>
                                            </datalist></td>
                                    </tr>
                                <?php }
...
<script>
    ...
    function editall()
    {
        $("#contacts tbody tr td").each(function() {
            ...
            if($(this).find("input[type=text],input[type=email],input[type=date]").css("display")=="none" &&  $(this).find("span").css("display")=="inline")
            {
                $(this).find("input[type=text],input[type=email],input[type=date]").show();
                $(this).find("span").hide();
            }
   ...
   function datatable()
   {
    ...
    function validertous()
    {
        ...
        var c=0;
        $("#contacts tbody tr td").each(function() {
                if(($(this).find("input[type=text]").css("display")=="inline" || $(this).find("input[type=text],input[type=email]").css("display")=="inline-block") &&  $(this).find("span").css("display")=="none")
                {
                    //console.log("oui");
                    $(this).find("input[type=text],input[type=email],input[type=date]").css("display","none");
                    $(this).find("span").html($(this).find("input[type=text],input[type=email],input[type=date]").val());
       ...
       var suivipar,nom,prenom,Tel,gsm,date,email,ville,villelycee,etablissmenet,anneuniv,telpere,marche,telmere,naturecontact,formationdemande,etablissement;
        for (var i = 0; i < aa.length; i++) {
            ...
            document.getElementById(id).style.display="none";
              suivipar=document.getElementById(aa[i].closest("tr").getAttribute("id")).cells[1].children[1].value;
                ...
                villelycee = document.getElementById(aa[i].closest("tr").getAttribute("id")).cells[8].children[1].value;
                etablissement = document.getElementById(aa[i].closest("tr").getAttribute("id")).cells[9].children[1].value;
                anneuniv = document.getElementById(aa[i].closest("tr").getAttribute("id")).cells[10].children[1].value;
                telpere = document.getElementById(aa[i].closest("tr").getAttribute("id")).cells[11].children[1].value;
                telmere = document.getElementById(aa[i].closest("tr").getAttribute("id")).cells[12].children[1].value;
                naturecontact = document.getElementById(aa[i].closest("tr").getAttribute("id")).cells[13].children[1].value;
                formationdemande = document.getElementById(aa[i].closest("tr").getAttribute("id")).cells[14].children[1].value;
             date = document.getElementById(aa[i].closest("tr").getAttribute("id")).cells[15].children[1].value;
            marche=document.getElementById(aa[i].closest("tr").getAttribute("id")).cells[16].children[1].value;
