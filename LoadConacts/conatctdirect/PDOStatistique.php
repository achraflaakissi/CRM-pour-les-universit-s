...
function array_to_xml_contact($contact_info, &$xml_contact_info) {
    foreach($contact_info as $key => $value) {
		...
		$sql = $this->db->query('SELECT * from contact_direct where Exp=0');
						//where (Typeexp="N" or Typeexp="M") and Exp =0
						$sql->execute();
						$contacts=$sql->fetchAll(PDO::FETCH_ASSOC);
						$xml = new SimpleXMLElement('<?xml version="1.0" ?><contacts_directs></contacts_directs>');
			...
			foreach($xml->xpath("//contacts_directs/contacts") as $node)
			{
			$node->Saisi_par=$this->getuserbyid($node->Saisi_par);
