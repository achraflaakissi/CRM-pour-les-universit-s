...
if(isset($_SESSION['user'])) {
    ...
    if($_SESSION['user']['profile']==sha1(md5('supercommercial'.$salt)) ){ 
        ...
        if(isset($_POST['campagnetest']))
        {
            ...
            $remplir=$remplir->updatecampagne_auto_cmp_appel("appel",$_POST['observation'],$_POST['ntel'],$_POST['nemail']
                ...
                ,$_POST['notenational'],$_POST['notegenerale'],$_POST['formation_demandee'],$_POST['status_rdv'],$_POST['etat_rdv'],$_POST['status'],$_POST['date_rdv'],$_POST['heure_rdv'],$_POST['inscription_rdv'],$_POST['type_contact']);
            echo $remplir;
            exit;
        }
        if(isset($_POST['campagne']))
        {
            ...
            require('content/controller/class.remlpiration.php');
            $remplir=new rempliration();
            $remplir=$remplir->get_auto_campagne_from_TR($_POST['type_contact'],$_POST['campagne']);
            echo $remplir;
            exit;
        }
                                        ...
                                        <div class="form-group">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" id="suivipar" style="width: 100%;">
                                                     <option   ></option>
                                                    <option value="46"  >Lamia EL AOUAME</option>
                                                    <optgroup label="Safi">
                                                        <?php $users = $bdd->query('select nom,prenom,id,centre from users where profile="commercial" and id <> 46 and centre = "Safi" ');
                                                        ...
                                                        <?php $users = $bdd->query('select nom,prenom,id,centre from users where profile="commercial" and id <> 46 and centre = "Essaouira" ');
                                                        $users =$users->fetchAll();
                                                        foreach($users as $u) {
                                                        ?>
                                                        <option value='<?php echo $u['id']; ?>'><?php echo $u['nom'].' '.$u['prenom']; ?></option>
                                                    ...
                                                    <optgroup label="El Jadida">
                                                        <?php $users = $bdd->query('select nom,prenom,id,centre from users where profile="commercial" and id <> 46 and centre = "El Jadida" ');
                                                        $users =$users->fetchAll();
                                                        foreach($users as $u) {
                                                        ?>
                                                        <option value='<?php echo $u['id']; ?>'><?php echo $u['nom'].' '.$u['prenom']; ?></option>
    ...
    <script>
        getfichecontact_auto_cmp_appel("<?php echo $_GET['id']; ?>",1,"","<?php echo $_GET['type_contact']; ?>");
